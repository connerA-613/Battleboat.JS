...
CONST.CSS_TYPE_HQ = 'hq';
CONST.CSS_TYPE_HQHIT = 'hqhit';
// Grid code:
...
Game.prototype.shoot = function(x, y, targetPlayer) {
	...
	if (targetGrid.isDamagedShip(x, y)) {
	...
	} else if (targetGrid.isHQ(x,y)) {

		if (targetPlayer === CONST.COMPUTER_PLAYER){

			targetGrid.updateCell(x,y, 'hqhit', targetPlayer);

			StatsPerGame.hqWon++;
			StatsPerGame.hqGames++;

			alert('Congratulations, you hit the HQ!');
			Game.gameOver = true;
			Game.stats.wonGame();
			Game.stats.syncStats();
			Game.stats.updateStatsSidebar();
		...
		} else if (targetPlayer === CONST.HUMAN_PLAYER) {

			StatsPerGame.hqGames++;

			alert('Yarr! The computer destroyed your HQ.');
			Game.gameOver = true;
			Game.stats.lostGame();
			Game.stats.syncStats();
			Game.stats.updateStatsSidebar();
...
Game.prototype.placementListener = function(e) {
	...
	if (self.placingOnGrid) {
		...
		if (successful) {
			...
			console.log(self.computerGrid);
			console.log("ship type: " + Game.placeShipType);
		...
		console.log("x: " + x);
		console.log("y: " + y);
		console.log(self.readyToPlay);
		if (self.humanGrid.isUndamagedShip(x, y) && self.readyToPlay == false) {
			currShip = self.humanFleet.findShipByCoords(x, y);
			let cells = currShip.getAllShipCells();
			for (var j = 0; j < cells.length; j++) {
				self.humanGrid.updateCell(cells[j].x, cells[j].y, 'empty', CONST.HUMAN_PLAYER);
			}
			Game.placeShipType = e.target.getAttribute('id');
			console.log("ship type: " + currShip.type);
			Game.usedShips[CONST.AVAILABLE_SHIPS.indexOf(currShip.type)] = 0;
			console.log(Game.usedShips);
			currShip.xPosition = -1;
			currShip.yPosition = -1;
			document.getElementById(currShip.type).removeAttribute('class');
			var el = document.getElementById('rotate-button');
...
Game.prototype.startGame = function(e) {
	...
	var hqBtn = document.getElementById("btn hq");
	classicBtn.setAttribute('class', 'hidden');
	hqBtn.setAttribute('class', 'hidden');
...
Game.prototype.restartGame = function(e) {
	...
	var classicBtn = document.getElementById("btn classic");
	var hqBtn = document.getElementById("btn hq");
	classicBtn.setAttribute('class', '');
	hqBtn.setAttribute('class', '');

	classicBtn.disabled = false;
	hqBtn.disabled = false;

	var step5 = document.getElementById("step5");
	step5.setAttribute("class", "hidden")
};
...
Game.prototype.classicMode = function(e) {
	var self = e.target.self;
	...
	for (let x = 0; x < Game.size; x++) {
		for (let y = 0; y < Game.size; y++){
			self.humanGrid.updateCell(x, y, 'empty', CONST.HUMAN_PLAYER);
		}
	}
	console.log(self.humanGrid);
	console.log(self.computerGrid);
...
Fleet.prototype.placeHQ = function() {
	this.playerGrid.cells[Math.floor(Game.size * Math.random())][Math.floor(Game.size * Math.random())] = CONST.TYPE_HQ;
}

Game.prototype.hqMode = function(e) {

	var x = Math.floor(Game.size * Math.random());
	var y = Math.floor(Game.size * Math.random());
	...
	// need access to grid
	// self.humanGrid.cells[x][y] = CONST.TYPE_HQ;
	self.humanGrid.updateCell(x, y, 'hq', CONST.HUMAN_PLAYER);
	self.computerFleet.placeHQ();
...
Game.prototype.init = function() {
	...
	var classicBtn = document.getElementById("btn classic");
	classicBtn.self = this;
	classicBtn.addEventListener('click', this.classicMode, false);

	var hqBtn = document.getElementById("btn hq");
	hqBtn.self = this;
	hqBtn.addEventListener('click', this.hqMode, false);
...
Grid.prototype.updateCell = function(x, y, type, targetPlayer) {
	...
	switch (type) {
		...
		case CONST.CSS_TYPE_SUNK:
			...
			break;
		case CONST.CSS_TYPE_HQ:
			this.cells[x][y] = CONST.TYPE_HQ;
			break;
		default:
			this.cells[x][y] = CONST.TYPE_EMPTY;
			break;
...
Grid.prototype.isHQ = function(x, y) {
	return this.cells[x][y] === CONST.TYPE_HQ;
};

Grid.prototype.isHQhit = function(x, y) {
	return this.cells[x][y] === CONST.TYPE_HQ_HIT;
};
...
Fleet.prototype.placeShipsRandomly = function() {
	...
	// this.playerGrid.cells[Math.floor(Game.size * Math.random())][Math.floor(Game.size * Math.random())] = CONST.TYPE_HQ;
	// this.computerGrid.cells[Math.floor(Game.size * Math.random())][Math.floor(Game.size * Math.random())] = CONST.TYPE_HQ;
		...
		for (var i = 0; i < this.shipLength; i++) {
			if (direction === Ship.DIRECTION_VERTICAL) {
				if (this.playerGrid.cells[x + i][y] === CONST.TYPE_SHIP ||
					...
					this.playerGrid.cells[x + i][y] === CONST.TYPE_HQ) {
					return false;
				}
			} else {
				if (this.playerGrid.cells[x][y + i] === CONST.TYPE_SHIP ||
					...
					this.playerGrid.cells[x][y + i] === CONST.TYPE_HQ) {
					return false;
				}
			}
		}
